     ...
     <td height="400" valign="top"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr> 
          <td align="left" background="imagenes/bckimg-1.png" class="texto-tsiscab">&nbsp;B&uacute;squeda 
            de Temas de Estudio &nbsp;por <strong>Tipo</strong>, mostrar resultados</td>
        </tr>
        <tr> 
          <td>&nbsp;</td>
        </tr>
...
if( (!$HTTP_GET_VARS[s]) || ($HTTP_GET_VARS[s] == "b") ) {
  tsiscab(); ?>
          ...
          <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
			...
			<?php
			if($st_tipo != "x" ) {
               $res = mysql_query("SELECT tipo FROM tipo WHERE idtipo='$st_tipo'") or die(mysql_error());
               if($fltipo=mysql_fetch_object($res)) {
			      echo $fltipo->tipo;
			   }
               mysql_free_result($res); unset($fltipo);
			} else { echo "Todos los Tipos de Estudio"; }
			?>
			</strong></td></tr>
              ...
              /* examino la página a mostrar y el inicio del registro a mostrar */
				 ...
				 /* ve condiciones segun parametro de busqueda */
					if( $st_tipo != "x" ) {
					    ...
					    $ssql.= " WHERE temas.idtipo='$st_tipo' AND realizado <> '1' ORDER BY fechasol DESC";
					} else { 
                        $ssql = "SELECT temas.id, idemp, temas.idtipo, temas.idarea, empresa, tipo, area, tituloestudio, fechasol FROM temas INNER JOIN admemp ON temas.idemp=admemp.id INNER JOIN tipo ON temas.idtipo=tipo.idtipo INNER JOIN area ON temas.idarea=area.idarea";
					    $ssql.= " WHERE realizado <> '1' ORDER BY temas.idtipo";
					}
                 $rs = mysql_query($ssql);
                    ...
                    <tr>
					   <td class="secc-ppal" colspan="5">
                         <table border="0" cellpadding="0" cellspacing="0" width="100%">
                           <tr>
						      <td colspan="2" class="texto-tsisr"><div id="lin-all">Tipo Estudio&nbsp;&nbsp;
							      <font color="#5475C6"><strong><?php echo $fila->tipo;?></strong></font></div></td>
                           </tr>
						   <?php $vtipo = $fila->idtipo;
						   while( $vtipo == $fila->idtipo ) { ?>
...
<?php
   ...
   {?> <tr><td height="10" valign="middle" class="info" style="padding-left:2px;"><div id="lin-sup">mostrar m&aacute;s registros en p&aacute;gina <?php
      for ($i=$minimo; $i<$pg; $i++){
         echo "<a href='".$HTTP_SERVER["PHP_SELF"]."?s=b&st_tipo=".$st_tipo."&pg=".$i."'>$i</a>&nbsp;";
      }
   ...
   if($pg<$total_paginas)
   {
     echo "&nbsp;<a href='".$HTTP_SERVER["PHP_SELF"]."?s=b&st_tipo=".$st_tipo."&pg=" .($pg+1). "'>";
