...
function _xmlTreeToJsTree(oNode) {
	...
	// check child nodes for attributes
	var cs = oNode.childNodes;
	var l = cs.length;
	for (var i = 0; i < l; i++) {
		if (cs[i].nodeName == "text" && text == null) 
			text = cs[i].childNodes[0].nodeValue;
		if (cs[i].nodeName == "action" && action == null)
			action = cs[i].childNodes[0].nodeValue;
		if (cs[i].nodeName == "icon" && icon == null)
			icon = cs[i].childNodes[0].nodeValue;
		if (cs[i].nodeName == "openIcon" && openIcon == null)
			openIcon = cs[i].childNodes[0].nodeValue;		
		if (cs[i].nodeName == "src" && src == null)
			src = cs[i].childNodes[0].nodeValue;	
		if (cs[i].nodeName == "target" && target == null)
			target = cs[i].childNodes[0].nodeValue;		
	}
