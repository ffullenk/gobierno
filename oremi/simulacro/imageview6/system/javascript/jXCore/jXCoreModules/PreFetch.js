	...
	jXCore.PreFetch.prototype = {
		...
		start: function(){						
			...
			if(oThis.FileList.length > 0){
				...
				if(oThis.checksize) {
					var iTotal = 0;
					for(var i = 0; i < oThis.FileList.length; i++){
						if(oThis.FileList[i].size == null) {
							if(!oThis.active) return; //bye-bii
							var oxhr = new jXCore.XMLHttpRequest();
							oxhr.open('HEAD', oThis.FileList[i].name, false);
							oxhr.send(null);
							
							var iSize = oxhr.getResponseHeader('Content-Length');
							if((iSize == null) || (iSize == '')){
								oThis.checksize = false; //error getting size, disabling checksize
								break;
							} else {
								oThis.FileList[i].size = parseInt(iSize); //store size
							}
						}
						iTotal += oThis.FileList[i].size;
					}
