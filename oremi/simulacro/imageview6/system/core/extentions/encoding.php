<?php
/*
Imageview Encoding Functions
*/
function ivencode($URI){
	$imageview = new imageview();
	$encode = $imageview->config->encoding();
	if($encode == 'uri'){
		return str_replace('%2F', '/', encodeURIComponent($URI));
	}else{
		return str_replace('%2F', '/', escape($URI));
	}
}

function ivdecode($URI){
	$imageview = new imageview();
	$encode = $imageview->config->encoding();
	if($encode == 'uri'){
