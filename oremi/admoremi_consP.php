                ...
                if ($Nniv == "2") {
                 // Conocer el IDFUENTE del userN
                    ...
                    <tr>
                      <td> 
                        <?                     
                       $con_comunas = mysql_query("SELECT idreg, idpro, idcom, comuna FROM Comuna WHERE idreg = '$region' AND idpro = '$provincia' ORDER BY idreg, idpro, idcom") or die(mysql_error());
                       //echo('<form action="consolidaProv.php" name="form" method="post">');
					   echo('<form action="newconsprov.php" name="form" method="post">');
// **                     echo('<form action="vemuestra.php" name="form" method="post">');
                       echo('<input type="hidden" name="userN" value="'.$userN.'">');
                       echo('<input type="hidden" name="cCons" value="'.$cCons.'">');

                      while($rcom = mysql_fetch_object($con_comunas)) {
                            $showcom = $rcom->comuna;
                            $fuentecomuna = $region.$provincia.$rcom->idcom; ?>
                        <table border=0 cellspacing=1 cellpadding=0 width=100% bgcolor=#F0F6F9>
                          <tr> 
                            <td bgcolor=#F0F6F9 colspan=2>Comuna: <strong class=topmenu><? echo $showcom; ?></strong> 
                            </td>
                          </tr>
                          <?                            
						  $con_eventos = mysql_query("SELECT Evento.idevento, idemergencia, idalfa, idfuente, idestadoevento, idestadoalfa, descinforme FROM Evento INNER JOIN InformeAlfa ON Evento.idevento=InformeAlfa.idevento WHERE idfuente = '$fuentecomuna' AND InformeAlfa.idestadoalfa = 0") or die(mysql_error());
...
?>
                          <tr>
                            <td bgcolor=#F8F8F8> 
                              <?                                       
							  echo('<input type="hidden" name="ncom" value="'.$rcom->idcom.'">');
                              echo('<input type="radio" name="com'.$rcom->idcom.'" value="'.$row->idemergencia.'">');
?>
                            </td>
                            <td bgcolor=#F8F8F8> 
                              <?   echo substr($row->descinforme,0,40). "... "; ?>
                            </td>
                          </tr>
                          <?                            }
                              mysql_free_result($con_eventos); ?>
                          <tr>
                            <td height=2 bgcolor=#F0F6F9></td>
                          </tr>
                          <tr>
                            <td></td>
                          </tr>
                    ...
                    <tr bgcolor=#F0F6F9>
                      <td align=center height=45 valign=middle bgcolor=#F8F8F8> 
                        <?                     if ( $xst == 1 ) { ?>
                        <input type="submit" name="consolidaprov" class="btn_med" value=" Consolidar "> 
                        <?                     } else { echo "<strong>No Existen Informes Comunales a Consolidar</strong>"; } ?>
                      </td>
                    </tr>
                    <?              } 
		      }				?>
                ...
                <td height="276" valign="top"> <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableborder">
                   ...
                   echo " $msg03 "; ?>
                    ...
                    // Consultar a Tabla Eventos en Proceso del user
                       ...
                       while($row=mysql_fetch_object($con_eventos)){ ?>
                          <tr> 
                            <td> <a href="admoremi_evenproc.php?id=<? echo $row->idevento; ?>&userN=<? echo $userN; ?>"> 
                              <? echo substr($row->desctipoevento,0,15). "... "; ?></a> 
                            </td>
                          </tr>
                          <tr>
                            <td></td>
