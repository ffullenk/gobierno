...
?>
    <div >
	   ...
	   <?php
		...
		$sqlEvento = "SELECT EVENTOALFA_ID, TITULOEVENTO, NRO_EVENTOS FROM EVENTOALFA WHERE EVENTOALFA_ID = \"$id\" ";
		$rsEvento->Open($sqlEvento);
		$nroEvento = $rsEvento->RecordCount();
		if( $nroEvento > 0 ) {
				$rsEvento->moveNext();
				$tituloEvento = $rsEvento->FieldByNumber(1);
				$nroEventos = $rsEvento->FieldByNumber(2);
		
echo "		<div id=\"mconregistro\">
					<p align=\"center\"><span style=\"font-size:14pt; font-family:Georgia,Verdana;line-height:18pt;\">Informes Alfas para el Evento <br /><a href=\"../../inicio.php\">". $tituloEvento . "</a></span></p>
...
$sqlAlfas = "SELECT INFORMEALFA_ID, EVENTOALFA_ID, TITULOINFORME, FECHA, HORA, NRO_INFORME, DESPACHADO FROM INFORMEALFA WHERE EVENTOALFA_ID=\"$id\" ";
				...
				if( $num_total_registros > 0 ) {
					...
					while($rsAlfas->moveNext() ) { 
						...
						if($i%2==0) { echo "<tr bgcolor=\"#ffffff\" valign=\"middle\">";} else {echo "<tr bgcolor=\"#ffffff\" valign=\"middle\">";}
						$i++;?>

<td>
<table width="100%" border="0" cellpadding="3" cellspacing="1">
  <tr valign="middle">
    <td style="background-color:#285C82;color:#FFFFFF;text-align:center;">Nro.Informe</td>
    <td style="background-color:#285C82;color:#FFFFFF;text-align:center;">Ocurrencia</td>
    <td rowspan="2" bgcolor="#DDCCBB">
	  <table width="100%" cellpadding="0" cellspacing="3">
	  <tr>
	  	<td>
			<?php if($rsAlfas->FieldByNumber(6)!="S") { ?>
					<form action="../actualiza/index.php" method="post">
		...
		<td><?php if($rsAlfas->FieldByNumber(6)!="S") { ?>	
						<form action="eliminar.php" method="post">
							<input type="hidden" name="id" value="<?php echo $id;?>" />
							<input type="hidden" name="alfa" value="<?php echo $rsAlfas->FieldByNumber(0);?>" />
							<input type="hidden" name="nroeventos" value="<?php echo $nroEventos;?>" />
		...
		<td >
			<form action="muestra.php" method="post">
				<input type="hidden" name="id" value="<?php echo $id;?>" />
				<input type="hidden" name="alfa" value="<?php echo $rsAlfas->FieldByNumber(0);?>" />
				<input type="submit" name="muestra" value="Ver" />
			</form>
		</td>
		<td>
			<?php if($rsAlfas->FieldByNumber(6)!="S") { ?>
					<form action="despacha.php" method="post">
						<input type="hidden" name="id" value="<?php echo $id;?>" />
						<input type="hidden" name="alfa" value="<?php echo $rsAlfas->FieldByNumber(0);?>" />
						<input type="submit" name="despacha" value="Despachar" />
					</form>	
			<?php } ?>
		</td>
	</tr>
  ...
  <tr bgcolor="#DDCCBB" valign="middle">
    <td align="center"><?php echo $rsAlfas->FieldByNumber(5) ." de ". $nroEventos;?></td>
    <td align="center"><?php echo fechaNuestra($rsAlfas->FieldByNumber(3)) ." ".$rsAlfas->FieldByNumber(4);?></td>
  </tr>
  <tr bgcolor="#C0D2DC">
    <td colspan="5"><?php echo $rsAlfas->FieldByNumber(2);?></td>
  </tr>
</table>
</td>
