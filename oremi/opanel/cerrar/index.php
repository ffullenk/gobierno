...
echo "		<div id=\"mconregistro\">
				<p>Usted visualiza en estos momentos, los Eventos que se encuentran en proceso y que han sido creado por Usted.
				
				Desde aqu&iacute;, podr&aacute; <strong>Cerrar un Evento</strong></p>
			...
			<div id=\"tablaRegistros\">
			  <table border=\"0\" >";
				...
				if( $num_total_registros > 0 ) {
					...
					$rsEventos->Open($ssql);
					echo "<tr><th class=\"tituloTabla\">Evento</th><th class=\"tituloTabla\">".$_aEventoColumna[$global_cg]."</th><th class=\"tituloTabla\">Ocurrencia</th><td>&nbsp;</tr>";
					$colorbg = "#F5F5F5";
					$i=0;
...
<form action="<?php echo $_aEventoPathCierra[$global_cg];?>index.php" method="post" name="edita" id="edita">
<td align="center">
<input type="hidden" name="id" value="<?php echo $rsEventos->FieldByNumber(0);?>" />
<input type="hidden" name="sesAlfa" value="<?php echo $sesAlfa;?>" />
<input type="submit" class="edita" value=" Cerrar Evento " />
</td>
</form>
...
<?php				} ?>
					<tr> 
						<td style="font-family:verdana; font-size:10px; color:#000;" colspan="4"> 
						<?php
...
echo "		        </td>
 			...
 			} else{ echo "<tr><td class=\"texto-contenido\" colspan=\"4\"> No se encuentran registros para visualizar</td></tr> ";} 
echo "
				</table>
			</div>
		</div>";
