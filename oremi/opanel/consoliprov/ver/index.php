...
?>
	...
	$sqlEvento = "SELECT EVENTOPROVINCIA_ID, TITULOEVENTO FROM EVENTOPROVINCIA WHERE EVENTOPROVINCIA_ID = \"$id\" ";
...
echo "		<div id=\"mconregistro\">
					<p align=\"center\"><span style=\"font-size:14pt; font-family:Georgia,Verdana;line-height:18pt;\">Consolidados Provinciales para el Evento<br />
					<a href=\"../../inicio.php\">".$tituloEvento."</a></span></p>
			  ...
			  <table border=\"0\" cellpadding=\"3\" cellspacing=\"1\" >";
				...
				$rsCons = new Recordset($SERVER, $DB, $USER, $PASSWORD);
				$sqlCons = "SELECT CONSOLIDAPROV_ID, TITULOEVENTO, FECHA, HORA 
								FROM CONSOLIDAPROV 
								WHERE EVENTOPROVINCIA_ID=\"$id\" ";

				$rsCons->Open($sqlCons);
				$num_total_registros = $rsCons->RecordCount();
				if( $num_total_registros > 0 ) {
					...
					$sqlCons.= " LIMIT ". $inicio . "," . $TAMANO_PAGINA .";";
					$rsCons->Open($sqlCons);
					echo "<tr><th class=\"tituloTabla\">Consolidado</th><th class=\"tituloTabla\">Ocurrencia</th><td></td></tr>";
					$colorbg = "#F5F5F5";
					$i=0;
					while($rsCons->moveNext() ) { 
					
						if($i%2==0) { echo "<tr bgcolor=\"#F5F5F5\" valign=\"middle\">";} else {echo "<tr bgcolor=\"#ffffff\" valign=\"middle\">";}
						$i++;?>
						
<td class="texto-contenido"><?php echo $rsCons->FieldByNumber(1);?></td>

<td class="texto-contenido"><?php echo fechaNuestra($rsCons->FieldByNumber(2)) ." ".$rsCons->FieldByNumber(3);?></td>

<form action="muestra.php" method="post" name="edita" id="edita" target="_blank">
<td align="center">
<input type="hidden" name="id" value="<?php echo $id;?>" />
<input type="hidden" name="consp" value="<?php echo $rsCons->FieldByNumber(0);?>" />
