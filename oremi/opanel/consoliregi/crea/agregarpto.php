...
?>
    <div >
	   ...
	   <?php
if(isset($maspto)) {
		// Grabar Punto Critico
		$rsDec = new Recordset($SERVER, $DB, $USER, $PASSWORD);
		$rsDec->FieldByInsert( "TPO", "'$sesAlfa'" );
		$rsDec->FieldByInsert( "IDENTIFICACION", "'$identificar'" );
		$rsDec->FieldByInsert( "UBICACION", "'$ubicar'" );
		$rsDec->FieldByInsert( "DANO", "'$danar'" );
		$rsDec->FieldByInsert( "ESTADOACTUAL", "'$estactual'" );
		$rsDec->FieldByInsert( "SOLUCION", "'$solucionar'" );
		$rsDec->FieldByInsert( "GESTION", "'$gestionar'" );
		$poneRegistro = $rsDec->execInsert( "TMPPC" , 1);
		...
		if($poneRegistro==true) {
			...
			echo "El Registro ha sido A&ntilde;adido de forma satisfactoria.<br /> 
					Por favor, para volver al formulario del <strong>Consolidado Regional</strong>, presione el bot&oacute;n <strong>Aceptar</strong>";
		} else {
