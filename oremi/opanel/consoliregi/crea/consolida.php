			...
			*/
			...
			if($poneRegistro==true) {
				...
				// --> Vamos a los Informes Alfas para marcarlos como ya consolidados de acuerdo al ID creado recientemente
						...
						foreach($sLista as $sitID) {
							...
							if($nroEncCom>0) {
								...
								while($rsEncCom->moveNext()) {								
									...
									$sqlAlfaD = "SELECT PERSONAS_DAMNIFICADAS, PERSONAS_ALBERGADAS, VIVIENDAS_DANO_MENOR, VIVIENDAS_DANO_MAYOR, VIVIENDAS_DESTRUIDA, VIVIENDAS_NOEVALUADAS, PERSONAS_AFECTADAS  
													FROM ALFADANOS 
					...
					$rsConsSit->FieldByInsert( "PERSONAS_AFECTADAS", "'".$rsAlfaD->FieldByNumber(6)."'" );
...
foreach($sLista as $sitID) {
   ...
   $qComu = "SELECT COMUNA FROM COMUNA AS C, INFORMEALFA AS I, ENCARGADOS AS E WHERE I.ENCARGADO_ID=E.ENCARGADO_ID AND E.COM_ID=C.COM_ID AND I.CONSOLIDAPROV_ID=\"".$iaC[$sitID]."\" ";
...
if($nroProvCom>0) {
  ...
  if($nroProv>0) {
    ...
    $asunto="Consolidado Regional: OREMI ".$nregion;
    $mail = new correo();
    $mail->setEmisor("Consolidado Regional",$emailEmisor);
$Mensaje_A_Responsables = " 
La Direccion Regional de ONEMI de la Region de Coquimbo informa que ha despachado un Consolidado Regional correspondiente al EVENTO ".$elTipoEventoEs.", ".$titulo.",  reportado por los municipios de: \n\n 
";
...
Deben ingresar a: http://www.gorecoquimbo.cl/oremi/opanel
...
$mail->putCorreo(_emailjefeturno_, "'".$asunto."'", $Mensaje_A_Responsables);
