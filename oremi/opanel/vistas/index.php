...
//
// Verificamos a que Region Pertenece el $global_qk
$rsRegion = new Recordset($SERVER, $DB, $USER, $PASSWORD);
$sqlRegion = "SELECT R.REGION_ID, REGION 
FROM REGION AS R, PROVINCIA AS P, COMUNA AS C, ENCARGADOS AS E 
WHERE ENCARGADO_ID=\"$global_qk\" AND 
E.COM_ID=C.COM_ID AND 
C.PROV_ID=P.PROV_ID AND 
P.REGION_ID=R.REGION_ID";
$rsRegion->Open($sqlRegion);
$nroRegion=$rsRegion->RecordCount();

if($nroRegion>0) {
	$rsRegion->moveNext();
	$IdRegion = $rsRegion->FieldByNumber(0);
	$NmRegion = $rsRegion->FieldByNumber(1);
}


$ssql="SELECT EVENTOREGION_ID, NRO_EVENTOS, TITULOEVENTO, FECHA_INICIO, HORA_INICIO, ESTADOEVENTO_ID 
FROM EVENTOREGION AS R, ENCARGADOS AS E, COMUNA AS C, PROVINCIA AS P 
WHERE P.REGION_ID = \"$IdRegion\" 
AND C.PROV_ID = P.PROV_ID 
AND C.COM_ID = E.COM_ID 
AND E.ENCARGADO_ID = R.ENCARGADO_ID ORDER BY FECHA_INICIO DESC";
...
<p style=\"background-color:#F0E9E1; padding:5px; border:1px solid #fff;font-family:Arial,Verdana; font-size:10pt;\">De acuerdo a nuestros registros, usted pertenece a la Regi&oacute;n de <strong>". $NmRegion . "</strong>. Por lo tanto, la informaci&oacute;n que a continuaci&oacute;n visualiza, corresponden a todos aquellos Eventos Regionales generados en su regi&oacute;n.</p>
			...
			<div id=\"tablaRegistros\">
			  <table border=\"0\" cellpadding=\"3\" cellspacing=\"1\" >";
				...
				if( $num_total_registros > 0 ) {
					...
					$rsEventos->Open($ssql);
					echo "<tr><th class=\"tituloTabla\">Evento</th><th class=\"tituloTabla\">".$_aEventoColumna[$global_cg]."</th><th class=\"tituloTabla\">Ocurrencia</th><td colspan=\"2\">&nbsp;</tr>";
					$colorbg = "#F5F5F5";
					$i=0;
...
<form action="informes.php" method="post" name="edita" id="edita">
<td align="center">
<input type="hidden" name="id" value="<?php echo $rsEventos->FieldByNumber(0);?>" />

<input type="submit" class="edita" value=" Ver Consolidados" />
</td>
</form>
