...
if( trim($_POST["cuenta"] != "") && trim($_POST["passwo"] != "") )
{
   ...
   //ahora, detectamos en la TABLA al usuario
	...
	$res = "SELECT PERSONA_ID FROM FUNCIONARIOS WHERE NICK_PERSONA='".$username."' AND PASS_PERSONA='".$pasword."' ";
	$resLogin->Open($res);
    ...
    if( $resLogin->RecordCount()>0 ) {
			...
			$t = time();
			$ta = date('Y-m-d H:i:s');
			$iduser = $resLogin->FieldByName("PERSONA_ID");
			$sesionid = md5(uniqid("$iduser:$t"));
			$qk = "$iduser:$sesionid";
			$kIP = ip();

			setcookie('ia', $qk); //, time() + 60*60*24*30, '', '', 0);

			$rEUPSes = new Recordset($SERVER, $DB, $USER, $PASSWORD);
			$rSsql="SELECT PERSONA_ID, ACTUALIP, ACTUALTIME FROM SESION_FUNC WHERE PERSONA_ID='".$iduser."' ";
			$rEUPSes->Open($rSsql);
			...
			if($rEUPSes->RecordCount()>0) {
				...
				$rUPSes->WhereByUpdate( "PERSONA_ID" , "$iduser" );
				$error = $rUPSes->execUpdate( "SESION_FUNC" , 1);

			} else {
				// para proceso posterior
				$rUPSes = new Recordset($SERVER, $DB, $USER, $PASSWORD);

				$rUPSes->FieldByInsert( "PERSONA_ID" , "'$iduser'" ); 
				$rUPSes->FieldByInsert( "SESIONID" , "'$sesionid'" ); 
				$rUPSes->FieldByInsert( "ACTUALIP" , "'$kIP'" ); 
				$rUPSes->FieldByInsert( "ACTUALTIME" , "'$ta'" ); 
				$error = $rUPSes->execInsert( "SESION_FUNC" , 1);
			}
