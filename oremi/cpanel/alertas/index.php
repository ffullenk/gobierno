...
<!-- Central -->
<?php
if(existenRegistros("ALERTA_ID","ALERTAS")) {
    // Existen Registros, Muestro Tabla con registros.
    echo "
		<div id=\"mconregistro\">
		    <form action=\"".$PHP_SELF."\" method=\"get\" name=\"buscar\">
			...
			<form action=\"nuevo.php\" method=\"post\" name=\"nuevo\" >
			  <input type=\"submit\" value=\" Nueva Alerta \" class=\"submit\" align=\"right\" />
			</form>
			
			<div id=\"tablaRegistros\">
			  <table border=\"0\" class=\"tablaAdmin\" >";
				/* Limito la busqueda */
				...
				$ssql = "SELECT ALERTA_ID, TPALERTA, TIEMPO, VARIABLE, ZONA, FECHA FROM ALERTAS";
				if($alerta != 0) {$ssql .= " WHERE TPALERTA='".$alerta."'"; }
				...
				if( $num_total_registros > 0 ) {
					...
					$rsSQL->Open($ssql);
					echo "<tr class=\"tituloTabla\"><th>Grado</th><th>Fecha</th><th>Variable</th><th>Zona</th><td colspan=\"2\">&nbsp;</tr>";
					while($rsSQL->moveNext() ) { ?>
						<tr valign="top">
							<td class="texto-contenido"><?php echo $_aAlerta[$rsSQL->FieldByNumber(1)];?></td>
							<td class="texto-contenido"><?php echo fechaNuestra($rsSQL->FieldByNumber(5)) . " ". $rsSQL->FieldByNumber(2);?></td>
							<td class="texto-contenido"><?php echo $_aVariable[$rsSQL->FieldByNumber(3)];?></td>
							<td class="texto-contenido"><?php echo $rsSQL->FieldByNumber(4);?></td>
							<td align="center">
							  <form action="edita.php" method="post" name="edita">
							    <input type="hidden" name="id" value="<?php echo $rsSQL->FieldByNumber(0);?>" />
								<input type="submit" name="btnedita" value="Editar" />
							  </form>	
							</td>
							<td align="center">
							  <form action="elimina.php" method="post" name="elimina">
							    <input type="hidden" name="id" value="<?php echo $rsSQL->FieldByNumber(0);?>" />
								<input type="submit" name="btnelimina" value="Eliminar" />
							  </form>	
