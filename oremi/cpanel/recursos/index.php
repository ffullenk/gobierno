		...
		<div id=\"mconregistro\">
			...
			<div id=\"tablaRegistros\">
			  <table border=\"0\" >";
				...
				$ssql = "SELECT RECURSO_ID, TIPORECURSO FROM RECURSOS";
				if($recurso != "") {$ssql .= " WHERE TIPORECURSO='".$recurso."'"; }
				...
				if( $num_total_registros > 0 ) {
					...
					$rsSQL->Open($ssql);
					echo "<tr><th class=\"tituloTabla\">Tipo de Recurso</th><td colspan=\"2\">&nbsp;</tr>";
					$i=0;
...
<?php				} ?>
					<tr> 
						...
						if($pg > 1) {
							echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".($pg-1)."&recurso=".$recurso."'>";
						...
						if ($total_paginas > 1) {
							for ($i=$minimo; $i<$pg; $i++){
								echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".$i."&recurso=".$recurso."'> $i</a>&nbsp;";
							}
							echo "[". $pg. "]&nbsp;";
...
echo "
	...
	echo "
			...
			<div id=\"zeroForm\">
			    <H5>Formulario Ingreso Detalle Recursos Involucrados</H5>
			    <form action=\"graba.php\" method=\"post\" name=\"agrega\">
				  <dl>
				      <dt>Tipo de Recurso:</dt>
					  <dd><input type=\"text\" name=\"recurso\" size=\"50\" maxlenght=\"50\" /></dd>
