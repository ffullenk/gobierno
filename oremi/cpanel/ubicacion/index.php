		...
		<div id=\"mconregistro\">
			...
			<div id=\"tablaRegistros\">
			  <table border=\"0\">";
				...
				$ssql = "SELECT TPUBICACION_ID, TPUBICACION FROM TP_UBICACION";
				if($ubicacion != "") {$ssql .= " WHERE TPUBICACION='".$ubicacion."'"; }
				...
				if( $num_total_registros > 0 ) {
					...
					$rsSQL->Open($ssql);
					echo "<tr><th class=\"tituloTabla\">Ubicación</th><td colspan=\"2\">&nbsp;</tr>";
					$i=0;
...
<?php				} ?>
					<tr> 
						...
						if($pg > 1) {
							echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".($pg-1)."&ubicacion=".$ubicacion."'>";
						...
						if ($total_paginas > 1) {
							for ($i=$minimo; $i<$pg; $i++){
								echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".$i."&ubicacion=".$ubicacion."'> $i</a>&nbsp;";
							}
							echo "[". $pg. "]&nbsp;";
...
echo "
	...
	echo "
			...
			<div id=\"zeroForm\">
			    <H5>Formulario Ingreso Detalle Ubicación Geográfica</H5>
			    <form action=\"graba.php\" method=\"post\" name=\"agrega\">
				  <dl>
				      <dt>Ubicación:</dt>
					  <dd><input type=\"text\" name=\"ubicacion\" size=\"30\" maxlenght=\"30\" /></dd>
