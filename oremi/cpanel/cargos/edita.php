...
<?php
  ...
  $nIDCargo = $_POST["id"];
	...
	$ssql = "SELECT CARGO_ID, CARGO FROM CARGOS WHERE CARGO_ID=\"".$nIDCargo."\" ";
	$rsSQL->Open($ssql);
	...
	if( $num_total_registros > 0 ) {
	  ...
	  echo "
			<div id=\"mconregistro\">
 			  ...
 			  <div id=\"zeroForm\">
			    <H5>Formulario Actualizacion Detalle Cargos Usuarios del Sistema</H5>
			    <form action=\"actualiza.php\" method=\"post\" name=\"actualiza\">
				<input type=\"hidden\" name=\"id\" value=\"".$rsSQL->FieldByNumber(0)."\" />
				  <dl>
				      <dt>Cargo:</dt>
					  <dd><input type=\"text\" name=\"cargo\" size=\"30\" maxlenght=\"30\" value=\"".$rsSQL->FieldByNumber(1)."\" /></dd>
