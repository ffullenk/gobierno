...
<!-- Central -->
<?php
if(existenRegistros("TPALFA_ID","TPALFAS")) {
    // Existen Registros, Muestro Tabla con registros.
    echo "
		<div id=\"mconregistro\">
		    <form action=\"".$PHP_SELF."\" method=\"get\" name=\"buscar\" id=\"formulario\">
		      <input type=\"text\" name=\"alfa\" size=\"30\" maxlenght=\"30\" />
		      <input type=\"submit\" value=\" Buscar \" class=\"btnsjv\" />
			</form>
			
			<form action=\"nuevo.php\" method=\"post\" name=\"nuevo\" >
			  <input type=\"submit\" value=\" Nuevo Estado del Informe Alfa \" class=\"crea\" align=\"right\" />
			</form>
			...
			<div id=\"tablaRegistros\">
			  <table border=\"0\" >";
				...
				$ssql = "SELECT TPALFA_ID, TPALFA FROM TPALFAS";
				if($alfa != "") {$ssql .= " WHERE TPALFA='".$alfa."'"; }
				...
				if( $num_total_registros > 0 ) {
					...
					$rsSQL->Open($ssql);
					echo "<tr><th class=\"tituloTabla\">Tipo de Estado</th><td colspan=\"2\">&nbsp;</tr>";
					$i=0;
					while($rsSQL->moveNext() ) { 						if($i%2==0) { echo "<tr bgcolor=\"#EAEADF\" valign=\"middle\">";} else {echo "<tr bgcolor=\"#ffffff\" valign=\"middle\">";}
						$i++;?>	
...
<?php				} ?>
					<tr> 
						...
						if($pg > 1) {
							echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".($pg-1)."&nivel=".$nivel."'>";
						...
						if ($total_paginas > 1) {
							for ($i=$minimo; $i<$pg; $i++){
								echo "<a href='".$_SERVER["PHP_SELF"]."?pg=".$i."&nivel=".$nivel."'> $i</a>&nbsp;";
							}
							echo "[". $pg. "]&nbsp;";
...
echo "
	...
	echo "
			...
			<div id=\"zeroForm\">
			    <H5>Formulario Ingreso Detalle Estados de los Informes Alfas</H5>
