...
if($loginCorrecto ) { 
    cabOremi();
	izqOremi();
	modOremi("E"); // Servicio Periodistico
?>
    <div >
       <div >
<!-- Central -->
...
$sqlAdmin = "SELECT NOMBRES, APELLIDOS, FONO, FONO2, MOVIL, MOVIL2, FAX, EMAIL, CUENTA, CARGO_ID  
				FROM ENCARGADOS 
...
if( $nro_cargos > 0 ) {
  while($rsCargos->moveNext()) {
  	if($rsCargos->FieldByNumber(0)== $rsAdmin->FieldByNumber(9)) {
  		echo "<option value=\"".$rsCargos->FieldByNumber(0)."\" selected>".$rsCargos->FieldByNumber(1)."</option>";  	
	} else {
