    ...
    // -->
      ...
      /* examino la página a mostrar y el inicio del registro a mostrar */
        ...
        /* construyo la sentencia SQL */
           $ssql = "SELECT idconsreg, ConsolidaRegion.idfuente, ocurrencia, descinforme, nrocons FROM ConsolidaRegion  WHERE idestadoevento=1 ORDER BY ocurrencia DESC LIMIT " . $inicio . "," . $TAMANO_PAGINA;
           $rs = mysql_query($ssql);
           ...
           while($row=mysql_fetch_object($rs)) {
				 ...
				 $con_fuente = mysql_query( "SELECT username FROM FUENTE WHERE idfuente = '$row->idfuente' AND nivel = '1' ") or die( mysql_error() );
				 if( $rfte = mysql_fetch_object( $con_fuente ) ) {
				     $fuente = $rfte->username;
				 }
				 mysql_free_result( $con_fuente);
?>
                      <tr bgcolor="#FfFfFf"> 
                        <td bgcolor="#F3F7F7"><a href="admoremi_vnacv.php?cCons=<? echo $row->idconsreg; ?>&fuente=<? echo $fuente; ?>" class="enlace"><? echo $fecha; ?></a></td>
                        <td bgcolor="#F3F7F7" align="center"><a href="admoremi_vnacv.php?cCons=<? echo $row->idconsreg; ?>&fuente=<? echo $fuente; ?>" class="enlace"><? echo $row->nrocons; ?></a></td>
                        <td bgcolor="#F3F7F7"><a href="admoremi_vnacv.php?cCons=<? echo $row->idconsreg; ?>&fuente=<? echo $fuente; ?>" class="enlace"><? echo $describe; ?></a></td>
                      </tr>
                      <?        } ?>
                    </table>
                  </DIV></td>
...
?>
          ...
          <td width="260" valign="top"> <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablebody">
              <tr> 
                <td width="260" height="150" valign="top"> <table width="95%" border="0" align="center" cellpadding="0" cellspacing="1" class="tableborderAlerta">
                    <tr> 
                      <td width="258" height="25" valign="middle" align="center" class="topmenu">Alertas 
                        Vigentes</td>
                    </tr>
                    <tr> 
