...
// Mostrar Informacion de la Tabla Consolidado Provincial que viene de antes
   $con_ninf = mysql_query("SELECT idconsprov, ConsolidaProvincia.idfuente, ocurrencia, desctipoevento, nrocons, resumen FROM ConsolidaProvincia INNER JOIN FUENTE ON ConsolidaProvincia.idfuente = FUENTE.idfuente WHERE FUENTE.username='$userN' AND idconsprov = '$cCons' ") or die(mysql_error());
  ...
  if($row=mysql_fetch_object($con_ninf)) {
   ...
   // Actualizar en Tabla Consolidado Provincial nrocons
      mysql_query("UPDATE ConsolidaProvincia SET nrocons = '$nroconsprov' WHERE idconsprov= '$cCons'") or Die(mysql_error());
...
echo "<HR color=#EFB520>";

for($i=1; $i <= $ncom; $i++) {
     if($i < 10) { $v= "0".$i; } else { $v = $i;}
     $vComuna = "com".$v;

     echo "Informacion en Tabla InformeAlfa del ID Actual:". $$vComuna ." Correspondiente a la Comuna ". $vComuna ."  [Comuna: ".$v ."] <BR>";
     echo "<HR>";
...
// idinfconsprov, idcom, pers_afectadas, pers_damnifica, pers_heridas, pers_muertas, pers_desaparec, pers_albergada, viv_danomenor, viv_danomayor, viv_destruirre, viv_noevaluada, serviciobasico, montodanos
