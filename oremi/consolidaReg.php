    ...
    // Mostrar Informacion de la Tabla Consolidado Provincial que viene de antes
       ...
       if($row=mysql_fetch_object($con_ninf)) {
         ...
         // Campos: idinfconsreg | idconsreg | idcons
            mysql_query("INSERT INTO InfConsReg(idconsreg, idcons) VALUES('$cCons','$nroconsreg')") or Die("Error ... Imposible Añadir Nro Consolidado Provincial Para Informe Consolidado en Tabla Informe Consolidado Regional.");
       ...
       for($i=1; $i <= $npro; $i++) {
        ...
        //          echo "Comuna: $nComuna <BR>";
         ...
         //     actualizar la tabla infconsprovincia
          ...
          $con_DanosP = mysql_query("SELECT * FROM ConsProvDanos WHERE idinfconsprov = '". $$vProvincia ."' ") or die("Error ... Imposible Acceder a Registro en Tabla Consolidado Provincial Daños.");
          ...
          while($rADec = mysql_fetch_object($con_Dec)) {
                mysql_query("INSERT INTO ConsRegDecisiones(idinfconsreg, idprov, accion, tiemporestablece) VALUES('$IDInfConsReg','$v','$rADec->accion','$rADec->tiemporestablece')") or Die("Error ... Imposible Añadir Registro en Tabla Consolidado Decisiones.");
          }
          ...
          while($rANec = mysql_fetch_object($con_Nec)) {
                mysql_query("INSERT INTO ConsRegNecesidades(idinfconsreg, idprov, cantidad, tipo, motivo) VALUES('$IDInfConsReg','$v','$rANec->cantidad','$rANec->tipo','$rANec->motivo')") or Die("Error ... Imposible Añadir Registro en Tabla Consolidado Necesidades.");
          }
          ...
          while($rARec = mysql_fetch_object($con_Rec)) {
                mysql_query("INSERT INTO ConsRegRecursos(idinfconsreg, idprov, idtiporecurso, descrirecurso, cantidadrecur, gastoestimado) VALUES('$IDInfConsReg','$v','$rARec->idtiporecurso','$rARec->descrirecurso','$rARec->cantidadrecur','$rARec->gastoestimado')") or die("Error ... Imposible Añadir Registro en Tabla Consolidado Recursos.");
          }
