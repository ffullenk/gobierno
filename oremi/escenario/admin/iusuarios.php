 ...
 if(estaActivo($userBackEnd, $passBackEnd)) {
   ...
   <td>
      <table border="0" width="90%" cellspacing="0" cellpadding="0" style="margin:20px auto; padding:0;" >
        <tr>
           <td align="right"><a href="form_agrega_usuarios.php">Agregar Usuario</a></td>
        </tr>
...
$sqlVal = "SELECT ormusr_id, nombre, email, rut, ID_INSTITUCION, tipo, estado FROM orm_usuarios ";
...
if( $num_total_registros > 0 ) {
   ...
   while($puntero = mysql_fetch_array($rsTabla) ) {
      ...
      <tr Bgcolor=\"".$color."\">
         ...
         <td>".veinst($puntero['ID_INSTITUCION'])."</td>
         <td>".$puntero['email']."</td>
	 ...
	 <td align=\"center\">";
         ...
         echo "</td>
         <td align=\"center\">
            <a href=\"form_actualiza_usuarios.php?idU=".$puntero['ormusr_id']."\" title=\"Actualizar Datos\">
...
function veinst( $institucion ) {
   $qInst = "SELECT INSTITUCION FROM orm_institucion WHERE ID_INSTITUCION=\"$institucion\" ";
   $rsInst = mysql_query($qInst);
   if(mysql_num_rows($rsInst) > 0 ) {
      $ptrInst = mysql_fetch_array($rsInst);
      return $ptrInst['INSTITUCION'];
   }
