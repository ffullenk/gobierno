...
function esAdministrador($usuario) {
   $query = "SELECT tipo FROM orm_usuarios WHERE rut='" . $usuario ."';";
   $rsQ = mysql_query($query);

   if(mysql_num_rows($rsQ) == 0 ) { return 0; }
   else {
     $puntero = mysql_fetch_array($rsQ);
     if($puntero['tipo']=="A") {return 1;} else { return -1;}
   }
}


function muestraUsuario($usuario) {
   $query = "SELECT nombre FROM orm_usuarios WHERE rut='" . $usuario ."';";
   $rsQ = mysql_query($query);
   if(mysql_num_rows($rsQ) == 0 ){return "";}
     else 
     {
        $puntero = mysql_fetch_array($rsQ);
...
function muestraID($usuario) {
  $query = "SELECT ormusr_id  
		  FROM orm_usuarios
		  WHERE rut='" . $usuario ."';";
...
function convertir_fecha_enpantala( $fFecha ) {
 $fecha = split("-", $fFecha );
return $fecha = $fecha[2] . "-" . $fecha[1] . "-" . $fecha[0];
}
?>