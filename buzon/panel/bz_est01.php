                          ...
                          <tr>
							...
							<!-- Parte  Central de la Pagina-->
								...
								<tr>
									<td>
									<div id="maincol" >
										<h1>Total Preguntas por Tipo y Tema, Respondidas y No Respondidas</h1>
									    <div align="right">
											<!-- no hay agregar -->
										</div>
									</div><!-- FIN MAINCOL -->
...
<?php
// >>>>> Total Preguntas por Tipo y Tema, Respondidas y No Respondidas <<<<<
// AND ACTIVADO=\"S\" 
$rsSQL=new Recordset($SERVER , $DB , $USER , $PASSWORD);
$ssql = "SELECT sum( 1  )  AS Preguntas, TIPO, TEMA, 
	SUM(  IF ( RESPUESTA =  \"N\", 1, 0 ) )  AS NO_RESPONDIDAS, 
	SUM(  IF ( RESPUESTA =  \"S\", 1, 0 ) )  AS RESPONDIDAS 
FROM BITACORA_C AS B, TIPO AS T, TEMAS AS M 
WHERE B.COD_TIPO=T.COD_TIPO AND B.COD_TEMA=M.COD_TEMA
GROUP  BY B.COD_TIPO, B.COD_TEMA";
$rsSQL->Open($ssql);
$conthit=0;
...
$enchit=$rsSQL->FieldByNumber(4); //respondidas
$hit[$conthit]=$enchit;  //repondidas
$totenchit+=$enchit; 	// respondidas

$NTipo[$conthit]=$rsSQL->FieldByNumber(1);
$Ntema[$conthit]=$rsSQL->FieldByNumber(2);

$noresp=$rsSQL->FieldByNumber(3);	//no respondidas
...
$totenc+=$subres;
	...
	$resenc.="<tr> 
			    ...
			    <td class=$Tclass rowspan=\"2\">$NTipo[$porres]</td>
			    <td class=$Tclass rowspan=\"2\">$Ntema[$porres]</strong> </td>
