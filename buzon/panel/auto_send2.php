...
<?php
	...
	$query = "SELECT MENSAJE, FECHA, TIPO, TEMA, B.COD_TEMA, TMP_BITC, concat(NOMBRES, ' ', PATERNO, ' ', MATERNO) as funcionario, EMAIL  
				...
				RESPUESTA = 'N'";
				//NOW() AS ahora, DATE_ADD(FECHA, INTERVAL 72 HOUR) AS ENMAS   
	$rsSql->Open($query);
	while( $rsSql->moveNext() ) {
		...
		echo " han pasado mas de 72 Horas <br />
	
		<div style=\"border-bottom:1px dashed #555E77;\" align=\"right\">&nbsp;</div>
		<br />";
		*/
		
		// Si han Pasado Mas de 72 Horas, reenviar el email a los responsables del tema en responderlo
		...
		while($rsTemas->moveNext())
		{
			...
			$Enlace_A_Responder = "http://www.gorecoquimbo.gob.cl/buzon/rpanel/index.php?CodTran=" . $rsSql->FieldByNumber(5);
			$mail = new correo();
			$emailEmisor="webmaster@gorecoquimbo.cl";
			$asunto = "Buzon Ciudadano :: [Email sin Responder: Pasaron 72 Horas]";
			$mail->setEmisor("Buzon Ciudadano :: GORE-COQUIMBO",$emailEmisor);
			$Mensaje_A_Responsables = "

El Email que usted recibe, es un reenvio puesto que han pasado 72 horas y no se ha dado respuesta al email enviado por el usuario: " . $rsSql->FieldByNumber(6) .", email " . $rsSql->FieldByNumber(7) . "
...
Le recordamos que existe un plazo de 72 horas para responder al usuario.
