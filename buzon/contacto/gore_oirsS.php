...
function comprobar_email($email){ 
	...
	*/
	...
	$vcap		= trim($_POST["vcap"]);
	
	$tipo		= $_POST["tipo"];
	$tema		= $_POST["tema"];
	$mensaje	= htmlentities($_POST["mensaje"]);
	
	//Obtenemos un ID para ver si viene por la via del usuarioregistrado o es primera vez que consulta y aprovechamos de anyadir los datos de la persona que consulta.
	
	$ddond = $_POST["ddond"];
	
	if($ddond==0) {
		// Es primera vez que consulta. Loa datos del usuario se graban completamente
		...
		if($vcap!=$cap) { $errores[$contador_error++] = "Error en datos del Captcha."; }
		...
		if($contador_error>0) { 
			...
			$query = "SELECT COD_PERS FROM PERSONA WHERE EMAIL = '".$email."';";
			$rsTabla->Open($query);
				...
				*/
		        ...
		        if ($error == true ) {
		           /* Buscamos al usuario ingresado para crear su correlacion en y de acuerdo a */
					...
					$query = "SELECT COD_PERS FROM PERSONA WHERE EMAIL = '".$email."';";
					$rsBscUs->Open($query);
					$nroPersonas = $rsBscUs->RecordCount();
					if($nroPersonas>0) { 
						$rsBscUs->moveNext();
		...
		} //END IF errores en campos
	} else { 
	// idPrsr, en el caso que ya este registrado, solo obtenemos el ID del usuario
	...
	if($CodPersona!=0) {
		...
		$sqlCTipo = "SELECT TIPO FROM TIPO WHERE COD_TIPO='".$tipo."'";
		...
		$sqlCTema = "SELECT TEMA FROM TEMAS WHERE COD_TEMA='".$tema."'";
		...
		$rsSaveMens->FieldByInsert( "COD_TIPO" , "'$tipo'" );
		$rsSaveMens->FieldByInsert( "COD_TEMA" , "'$tema'" );
		...
		if ($error == true ) {
			...
			$query = "SELECT COD_TEMA, F.COD_FUNCIONARIO, EMAIL 
					...
					WHERE Q.COD_FUNCIONARIO = F.COD_FUNCIONARIO AND 
						COD_TEMA='".$tema."' AND F.ACTIVADO='S';";
			$rsTemas->Open($query);
			...
			while($rsTemas->moveNext())
			{
				...
				$asunto="Buzon Ciudadano";
			
				$mail = new correo();
				$mail->setEmisor("Buzon Ciudadano",$emailEmisor);
...
".$Enlace_A_Responder." Recuerde que existe un plazo de 72 horas para responder al usuario.
...
Buzón Ciudadano :: Gobierno Regional de Coquimbo";
				...
				for($i=0; $i<count($email_responsable);$i++) {
					$mail->putCorreo($email_responsable[$i], "Buzon Ciudadano Gobierno Regional de Coquimbo", $Mensaje_A_Responsables);
				 }	
				 $mail->sendCorreo();
...
/*			foreach($email_responsable as $email_a) {			
			...
			/* Debemos Informar al usuario quien consulta, que en un plazo no mayor a 72 debiesen responder. */
...
en un plazo no superior a 72 horas, recibira respuesta a su consulta.
...
";
				$mail_A_Usuario->putCorreo($email, "Buzon Ciudadano Gobierno Regional de Coquimbo",	$Mensaje_A_Usuario);
		...
		// El Cod. Persona nos podria arrojar 0 ¿...? que pena, que horrible.
		echo('<div align="center">
		Ha Ocurrido un error inesperado, por favor, vuelva a intentar.<br />
						<input class="formbutton" name="atras" type="button" value="Volver" onClick="javascript:history.back();"></div>');
	}
?>
</body>
</html>
