...
if(trim($_POST["cuenta"] != "") && trim($_POST["passwo"] != ""))
{
   ...
   //ahora, detectamos en la TABLA al usuario
     ...
     if( $user_obj = mysql_fetch_object($res) ) {
			...
			$qk = "$iduser:$sesionid";
			setcookie('QrB', $qk); //, time() + 60*60*24*30, '', '', 0);
	       //Ahora, actualizamos la información en la Tabla
			...
			mysql_close($link);
			if(empty($ticket)) {
				echo "<HTML><HEAD><META HTTP-EQUIV=Refresh CONTENT='0; URL=rhome.php'></HEAD><Body BgColor=White></Body></HTML>";
				die();
			} else {
				echo "<HTML><HEAD><META HTTP-EQUIV=Refresh CONTENT='0; URL=rhome.php?tket=$ticket'></HEAD><Body BgColor=White></Body></HTML>";
				die();
			}
      } else {
