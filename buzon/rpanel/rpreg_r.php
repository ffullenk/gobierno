					    ...
					    <!-- Seccion Central -->
                          ...
                          <tr>
							...
							<!-- Parte  Central de la Pagina-->
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
								   <td class="texto-ver"><form id="FMuestra" method="get" action="rpreg_r.php">
								   <table width="550" border="0" align="center">
  									<tr class="texto-contenido">
										...
										$qryTema = "SELECT T.COD_TEMA, TEMA 
													FROM TEMAS AS T, QRESPONDE AS Q 
													WHERE T.COD_TEMA = Q.COD_TEMA AND 
													COD_FUNCIONARIO = '". $global_qk."';";
										$rsTema->Open($qryTema);
										...
										<td><input type="submit" value="Ver" class="btn" onMouseOver="this.className='btn btnhov'" onMouseOut="this.className='btn'"></td>
	  								</tr>
									</table></form>										
							       </td>
								</tr>
								   ...
								   <tr><td height="5"></td></tr>
									<?php
									  ...
									  /* examino la página a mostrar y el inicio del registro a mostrar */
										...
										$ssql = "SELECT COD_BITC, concat( NOMBRES, ' ', PATERNO, ' ', MATERNO ) , MENSAJE, DATE_FORMAT(FECHA, '%d/%m/%Y %H:%i:%s') as enviado, BC.COD_TIPO, BC.COD_TEMA 
												FROM BITACORA_C AS BC, PERSONA AS P, QRESPONDE AS Q 
												WHERE BC.COD_PERS = P.COD_PERS AND 
														Q.COD_TEMA = BC.COD_TEMA AND 
														Q.COD_FUNCIONARIO = '". $global_qk."'";
												if($tipo != 0) { $ssql.= " AND BC.COD_TIPO = '". $tipo ."' "; }
												if($tema != 0) { $ssql.= " AND BC.COD_TEMA = '". $tema ."' "; }															
										
										/*  
										$ssql = "SELECT COD_BITC, concat(NOMBRES, \"  \",PATERNO, \"  \", MATERNO), MENSAJE, FECHA 
													FROM BITACORA_C AS BC, PERSONA AS P  
													WHERE BC.COD_PERS = P.COD_PERS ";
													if($tipo != 0) { $ssql.= " AND BC.COD_TIPO = '". $tipo ."' "; }
													if($tema != 0) { $ssql.= " AND BC.COD_TEMA = '". $tema ."' "; }*/
										$ssql.= " AND RESPUESTA = 'S' ORDER BY FECHA DESC";
										$rsSQL->Open($ssql);
										...
										if( $num_total_registros > 0 ) {
											...
											<tr><td>
												...
												<tr>
													<td width="91" class="texto-tablas">Fecha</td><td class="texto-contenido"><strong><?php echo $rsSQL->FieldByNumber(3);?></strong></td>
													<td width="178" bgcolor="#FFFFFF" align="right">
														<form id="formlist" method="post" action="rvrespuesta.php">
														<input type="hidden" name="id" value="<?php echo $rsSQL->FieldByNumber(0);?>">
														<input type="hidden" name="global_qk" value="<?php echo $global_qk;?>">
										                  <input type="submit" value="Ver Respuesta" class="btn" onMouseOver="this.className='btn btnhov'" onMouseOut="this.className='btn'">
														</form>
													</td>
												</tr>
												<tr> 
												    <td class="texto-tablas">Usuario</td>
												    <td colspan="2" class="texto-contenido"><strong><?php echo $rsSQL->FieldByNumber(1);?></strong></td>
												</tr>
<?php
$rsTipo=new Recordset($SERVER , $DB , $USER , $PASSWORD);
